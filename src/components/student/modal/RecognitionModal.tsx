...
import { addStudentRecognition } from '../../../api/studentApi';
...
const RecognitionModal: React.FC<StudentCertModalProps> = ({ userId, isOpen, onClose, onRecogAdded}) => {
  const [newRecogData, setNewRecogData] = useState<StudentCertAndRecogModel>(iniStudentCertAndRecog);
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null);
  const [attachedFile, setAttachedFile] = useState<File>(); // Array to handle multiple images
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const formData = new FormData();
    formData.append('id', newRecogData.id.toString());
    formData.append('userId', userId.toString());
    formData.append('name', newRecogData.name);
    formData.append('certRecogType', '2');
    if (attachedFile) {
    ...
    try {
      await addStudentRecognition(formData);
      setToast({ message: 'Recogination added successfully', type: 'success' });      
      setTimeout(() => {
        setToast(null);
        onRecogAdded();
        onClose();
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setNewRecogData({ ...newRecogData, name: e.target.value });
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div className="relative bg-white p-8 rounded-lg shadow-lg w-[50%] md:w-[25%]">
          <div className="flex items-center justify-between p-1 border-b rounded-t dark:border-gray-600">
            ...
            <h3 className="text-xl font-medium text-gray-900 dark:text-white">Add Your Recognition</h3>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="max-w-lg mx-auto">
            ...
            <div className="mb-4">
                <label htmlFor="attachementFile" className="block text-gray-700 text-sm font-bold mb-2">
                  Cover Photo One
                </label>                
                <input
                  type="file"
